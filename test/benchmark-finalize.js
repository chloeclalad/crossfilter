...
function updateDisplay() {
  coreops.finalize(dates.all()); // update the date chart
  coreops.finalize(days.all()); // update the day-of-week chart
  coreops.finalize(hours.all()); // update the hour-of-day chart
  coreops.finalize(amounts.all()); // update the amount histogram
  coreops.finalize(all.value()); // update the summary totals
  coreops.finalize(date.top(40)); // update the payment list
  ++k; // count frame rate
}
